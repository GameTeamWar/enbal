...
export default function QuoteDetailModal({
  ...
  // Helper function to check if quote is cancelled
  ...
  // Helper function to determine if document upload should be shown
  const shouldShowDocumentUpload = (quote: any) => {
    return quote.status === 'responded' && 
           !quote.documentUrl && 
...
TUTAR: ${quote.price ? new Intl.NumberFormat('tr-TR', { style: 'currency', currency: 'TRY' }).format(parseFloat(quote.price)) : 'Belirtilmemiş'}
    ...
    <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-lg">
      <div className="flex items-center">
        ...
        <span className="text-red-800 font-medium">
          Bu teklif iptal edilmiştir. İşlem yapılamaz.
        </span>
      </div>
      {quote.rejectionReason && (
        <p className="text-red-700 text-sm mt-2">
          <strong>İptal Nedeni:</strong> {quote.rejectionReason}
        </p>
      )}
      {quote.customerRejectionReason && (
        <p className="text-red-700 text-sm mt-1">
          <strong>Müşteri İptal Nedeni:</strong> {quote.customerRejectionReason}
        </p>
      )}
    </div>
  )}
  ...
  <div className="flex flex-wrap gap-3">
    {/* ✅ CEVAPLA - Sadece pending ve iptal edilmemiş */}
    ...
    {/* ✅ İPTAL ET - Sadece iptal edilmemiş ve belge yüklenmemiş */}
    ...
    {/* ✅ BELGE YÜKLE - Sadece gerekli durumlarda */}
